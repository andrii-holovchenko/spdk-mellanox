...
static bool
nvme_bdev_channel_has_io_path(struct nvme_bdev_channel *nbdev_ch,
			      struct nvme_io_path *io_path)
{
	struct nvme_io_path *p;

	STAILQ_FOREACH(p, &nbdev_ch->io_path_list, stailq) {
		if (p == io_path) {
			return true;
		}
	}
				  ...
				  const struct spdk_nvme_cpl *cpl)
{
	...
	if (!nvme_bdev_channel_has_io_path(nbdev_ch, io_path)) {
		/* I/O path was removed while processing this I/O. */
		bdev_nvme_clear_current_io_path(nbdev_ch);
		bio->io_path = NULL;
...
bdev_nvme_create_qpair(struct nvme_qpair *nvme_qpair)
{
	...
	NVME_CTRLR_NOTICELOG(nvme_ctrlr, "Connecting qpair %p:%u started.\n",
...
bdev_nvme_failover_trid(struct nvme_ctrlr *nvme_ctrlr, bool remove, bool start)
{
	...
	if (next_path == NULL || (!start && next_path->is_failed)) {
		/* No alternate path or all alternate paths failed. */
		return false;
	}
	...
	if (start) {
		/* Reset the status of alternate paths to try all. */
		TAILQ_FOREACH_FROM(next_path, &nvme_ctrlr->trids, link) {
			next_path->is_failed = false;
		}
	}
...
nvme_ctrlr_disconnect(struct nvme_ctrlr *nvme_ctrlr, nvme_ctrlr_disconnected_cb cb_fn)
{
	...
	NVME_CTRLR_NOTICELOG(nvme_ctrlr, "Start disconnecting ctrlr.\n");
...
bdev_nvme_reset_destroy_qpair(struct spdk_io_channel_iter *i)
{
	...
	if (qpair != NULL) {
		NVME_CTRLR_NOTICELOG(nvme_qpair->ctrlr,
				     "Start disconnecting qpair %p:%u.\n",
				     qpair, spdk_nvme_qpair_get_id(qpair));
...
bdev_nvme_reset_create_qpairs_done(struct spdk_io_channel_iter *i, int status)
{
	...
	if (status == 0) {
		NVME_CTRLR_NOTICELOG(nvme_ctrlr, "qpairs were created after ctrlr reset.\n");

		bdev_nvme_reset_complete(nvme_ctrlr, true);
	} else {
		NVME_CTRLR_ERRLOG(nvme_ctrlr, "qpairs were failed to create after ctrlr reset.\n");
...
bdev_nvme_reset_check_qpair_connected(void *ctx)
{
	...
	if (ctrlr_ch->reset_iter == NULL) {
		...
		NVME_CTRLR_NOTICELOG(nvme_qpair->ctrlr,
				     "qpair was failed to connect. reset is being aborted.\n");
	...
	NVME_CTRLR_NOTICELOG(nvme_qpair->ctrlr, "qpair %p:%u was connected.\n",
			     qpair, spdk_nvme_qpair_get_id(qpair));
...
bdev_nvme_reset_create_qpair(struct spdk_io_channel_iter *i)
{
	...
	if (rc == 0) {
		...
		NVME_CTRLR_NOTICELOG(nvme_qpair->ctrlr, "Start checking qpair %p:%u to be connected.\n",
				     qpair, spdk_nvme_qpair_get_id(qpair));
...
bdev_nvme_reconnect_ctrlr_poll(void *arg)
{
	...
	spdk_poller_unregister(&nvme_ctrlr->reset_detach_poller);
	if (rc == 0) {
		NVME_CTRLR_NOTICELOG(nvme_ctrlr, "ctrlr was connected. Create qpairs.\n");
		...
		NVME_CTRLR_ERRLOG(nvme_ctrlr, "ctrlr could not be connected.\n");
...
bdev_nvme_reset_ctrlr(struct spdk_io_channel_iter *i, int status)
{
	...
	NVME_CTRLR_NOTICELOG(nvme_ctrlr, "qpairs were deleted.\n");
...
{
	NVME_CTRLR_NOTICELOG(nvme_ctrlr, "Delete qpairs for reset.\n");
...
bdev_nvme_reset_unsafe(struct nvme_ctrlr *nvme_ctrlr, spdk_msg_fn *msg_fn)
{
	...
	/* When bdev_nvme_reset() is called, it is not certain that the current path failed.
	 * Hence, reset the status of all paths including the current path.
	 */
	TAILQ_FOREACH(path_id, &nvme_ctrlr->trids, link) {
		path_id->is_failed = false;
	}
...
static void
bdev_nvme_unfreeze_bdev_channel_done(struct spdk_io_channel_iter *i, int status)
{
...
static void
bdev_nvme_unfreeze_bdev_channel(struct spdk_io_channel_iter *i)
{
	struct spdk_io_channel *_ch = spdk_io_channel_iter_get_channel(i);
...
static void
bdev_nvme_freeze_bdev_channel_done(struct spdk_io_channel_iter *i, int status)
{
	struct nvme_bdev_io *bio = spdk_io_channel_iter_get_ctx(i);
	struct spdk_bdev_io *bdev_io = spdk_bdev_io_from_ctx(bio);
	struct nvme_bdev_channel *nbdev_ch;
...
static void
bdev_nvme_freeze_bdev_channel(struct spdk_io_channel_iter *i)
{
	struct spdk_io_channel *ch = spdk_io_channel_iter_get_channel(i);
...
bdev_nvme_submit_request(struct spdk_io_channel *ch, struct spdk_bdev_io *bdev_io)
{
	...
	/* zcopy end must use existing path for decreasing outstanding_zcopy_reqs */
			...
			spdk_nvme_accel_step_cb cb_fn, void *cb_arg)
{
	...
	return spdk_accel_append_crc32c((struct spdk_accel_sequence **)seq, group->accel_channel, dst, iovs, iovcnt,
		 ...
		 uint32_t prchk_flags, void *ctx)
{
	...
	disk->max_rw = (disk->max_segment_size / disk->blocklen);
			       ...
			       struct nvme_async_probe_ctx *ctx)
{
	...
	while (nvme_ns != NULL) {
		...
		if (spdk_nvme_ctrlr_is_active_ns(ctrlr, nvme_ns->id)) {
			...
			if (nvme_ns->ns != ns) {
				NVME_CTRLR_NOTICELOG(nvme_ctrlr, "NSID %u was reconstructed while reconnect\n",
						     nvme_ns->id);
				nvme_ns->ns = ns;
			}
...
bdev_nvme_set_opts(const struct spdk_bdev_nvme_opts *opts)
{
	...
	if (opts->poll_group_requests != 0) {
		drv_opts.poll_group_requests = opts->poll_group_requests;
	}
				 ...
				 struct spdk_nvme_zcopy_io *zcopy_io)
{
	...
	/* @todo: in other IO types completion we handle path errors and do path switch.
...
bdev_nvme_readv_zcopy_end(struct nvme_bdev_io *bio)
{
	...
	if (rc != 0) {
		SPDK_ERRLOG("readv zcopy end failed: rc = %d\n", rc);
	}
	return rc;
}
...
bdev_nvme_readv_zcopy_start(struct nvme_bdev_io *bio)
{
	...
	SPDK_DEBUGLOG(bdev_nvme, "zcopy read %" PRIu64 " blocks with offset %#" PRIx64 "\n",
		      lba_count, lba);
	...
	if (rc != 0 && rc != -ENOMEM) {
		SPDK_ERRLOG("readv zcopy start failed: rc = %d\n", rc);
	}
	return rc;
}
